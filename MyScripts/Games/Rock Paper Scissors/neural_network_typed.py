...
import numpy as np
# typing is used to add type hints.
...
def sigmoid_output_derivative(x: np.ndarray) -> np.ndarray:
...
class NeuronLayer:
	...
	def think(self, inputs: Sequence[float]) -> np.ndarray:
...
class NeuralNetwork:
	...
	def __init__(self, layers: Sequence[NeuronLayer]) -> None:
		"""Makes a neural network with the specified layers."""

		# Storing the layers.
		self.layers = layers

	def think(self, inputs: Sequence[float]) -> List[np.ndarray]:
	...
	def adjust(self, inputs: Sequence[float], outputs: Sequence[np.ndarray], correct_output: Sequence[int]) -> None:
		"""Adjusts the weights."""
...
class RPSbot(NeuralNetwork):
	...
	def __init__(self, layers: Sequence[NeuronLayer], move_depth: int) -> None:
	...
	@staticmethod
	def get_move_input(prompt: str) -> int:
